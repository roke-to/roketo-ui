...
import {$subscriptions} from '~/entities/wallet';
...
export function SubscriptionFilters({className}: {className: string}) {
  const [showInput, setShowInput] = useState(false);
  const [showCompactFilterModal, setShowCompactFilterModal] = useState(false);

  const [sorting, {status, text: filterText}, statusFilterCounts, isSmallScreen] = useUnit([
    $subscriptionSort,
    $subscriptionFilter,
    $statusFilterCounts,
    $isSmallScreen,
  ...
  const isEmptyList = useStoreMap($subscriptions, (items) => items.length === 0);
  ...
  return (
    <div className={cn(styles.root, className)}>
      ...
      <Filter
        ...
        label="Sort by:"
        active={sorting}
        onChange={changeSubscriptionSort}
        isDisabled={isEmptyList}
