...
export async function initRoketo({account}: {account: Account}): Promise<Roketo> {
  ...
  const [roketoUserAccount, dao] = await Promise.all([roketoUserAccountPromise, daoPromise]);
