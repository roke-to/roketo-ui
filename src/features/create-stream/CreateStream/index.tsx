...
const StreamToWallet = ({onFormCancel, onFormSubmit, submitting}: CreateStreamProps) => {
...
const StreamToNFT = ({onFormCancel, onFormSubmit, submitting}: CreateStreamProps) => {
  const tokens = useStore($listedTokens);
  const accountId = useStore($accountId);
  const [submitError, setError] = useState<Error | null>(null);
  const [streamAmount, setStreamAmount] = useState(0);
...
export const CreateStream = ({onFormCancel, onFormSubmit}: CreateStreamP) => {
  const nft = true;
