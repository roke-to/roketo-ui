...
export function useAccount(): SWRResponse<RoketoAccount> {
  ...
  const swr = useSWR(['account', auth.accountId], roketo.api.getCurrentAccount, {
...
export function useLegacyStreams({account}: {account?: RoketoAccount}) {
  ...
  const swr = useSWR(
    () => {
      const key = account ? ['streams', account.account_id, account.last_action] : false;
    ...
    async () => {
      ...
      const identified = identifyStreamsDirection(fetchedStreams, auth.accountId).filter(
        (stream) => !isDead(stream),
      );

      balances.current = {
        values: identified.reduce(
          (balancesDraft, stream) =>
            Object.assign(balancesDraft, {
  ...
  React.useEffect(() => {
    const pollerId = setTimeout(() => {
      if (!swr.data || interpolationsLeft.current <= 0) {
        ...
        const interpolateAll = (stream: LegacyRoketoStream) =>
          interpolateStream(stream, balances.current.values[stream.id], balances.current.cachedAt);
...
export function useLegacySingleStream(streamId: string, account?: RoketoAccount) {
  ...
  const swr = useSWR<LegacyRoketoStream>(
    () => {
      const key = account ? ['stream', streamId, account.account_id, account.last_action] : false;
      return key;
    },
    async () => {
    ...
    if (stream.status === STREAM_STATUS.ACTIVE && !isCompleted) {
      const pollerId = setTimeout(() => {
        if (!swr.data || interpolationsLeft.current <= 0) {
          ...
          swr.mutate(interpolateStream(swr.data, balance.current.value, balance.current.cachedAt), {
            revalidate: false,
          });
        }
      }, 1000);
...
export function streamViewData(stream: LegacyRoketoStream, withExtrapolation: boolean = true) {
  ...
  const availableToWithdraw = withExtrapolation
    ? new BigNumber(stream.available_to_withdraw)
    : new BigNumber(0);
