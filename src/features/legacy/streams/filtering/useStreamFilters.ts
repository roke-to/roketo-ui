...
export function useStreamFilters(streams: LegacyRoketoStream[] | undefined) {
  ...
  const statusOptions = useMemo(
    () => ({
      ...
      [STREAM_STATUS_FILTER[STREAM_STATUS.INITIALIZED]]: (stream: LegacyRoketoStream) =>
        stream.status === STREAM_STATUS.INITIALIZED,
      [STREAM_STATUS_FILTER[STREAM_STATUS.ACTIVE]]: (stream: LegacyRoketoStream) =>
        stream.status === STREAM_STATUS.ACTIVE,
      [STREAM_STATUS_FILTER[STREAM_STATUS.PAUSED]]: (stream: LegacyRoketoStream) =>
  ...
  const directionOptions = useMemo(
    () => ({
      ...
      [STREAM_TYPE_FILTER.INCOMING]: (stream: LegacyRoketoStream) =>
        auth.accountId === stream.receiver_id,
      [STREAM_TYPE_FILTER.OUTGOING]: (stream: LegacyRoketoStream) =>
        auth.accountId === stream.owner_id,
    }),
    [auth.accountId],
  );
